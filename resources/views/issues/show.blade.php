@php
    $type = \TomatoPHP\TomatoCategory\Models\Type::where('for', 'issues')->where('type', 'types')->where('key', $model->type)->first();
    $parent = \TomatoPHP\TomatoTasks\Models\Issue::find($model->parent_id);
@endphp
<x-tomato-admin-layout>
    <div class="flex justify-between">
        <div class="flex justify-start gap-2">
            @if($parent)
                @php $parentType = \TomatoPHP\TomatoCategory\Models\Type::where('for', 'issues')->where('type', 'types')->where('key', $parent->type)->first(); @endphp
    ...
    <div class="grid grid-cols-12 gap-4 my-4 ">
       ...
       <div class="col-span-4 overflow-y-scroll h-max-screen bg-white rounded-lg border border-gray-200 shadow-sm p-4">
           @php $statues = \TomatoPHP\TomatoCategory\Models\Type::where('for', 'issues')->where('type', 'status')->get(); @endphp
            <div class="flex flex-col gap-4">
                ...
                <div class="flex justify-start gap-4">
                    ...
                    @endif
                    @if(class_exists(\TomatoPHP\TomatoTimer\Models\Timer::class))
                    <div>
    ...
    <div class="overflow-y-scroll h-max-screen bg-white rounded-lg border border-gray-200 shadow-sm p-4 my-4">
        <x-splade-table :for="(new \TomatoPHP\TomatoTasks\Tables\IssueTable($model->childrens()))">
